...
__device__ __forceinline__ uint32_t LCGTAUS(uint4& state) {
    ...
    state.w = state.w * 1664525 + 1013904223U;
...
//        3            | .337 bits
//        4            | .201 bits
__global__ void InitRandom(uint32_t* sort, uint32_t andCount, uint32_t seed, uint32_t size) {
    const uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    uint4 prngState = make_uint4(idx * 4 * seed, (idx * 4 + 1) * seed, (idx * 4 + 2) * seed,
                                 (idx * 4 + 3) * seed);
    LCGTAUS(prngState);
...
__global__ void InitRandom(uint32_t* sort, uint32_t* sortPayload, uint32_t andCount, uint32_t seed,
                           uint32_t size) {
    const uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    uint4 prngState = make_uint4(idx * 4 * seed, (idx * 4 + 1) * seed, (idx * 4 + 2) * seed,
                                 (idx * 4 + 3) * seed);
...
__global__ void InitFixedSegLengthDescendingValue(uint32_t* sort, uint32_t segLength,
...
template <class V>
__device__ void SegRandomTemplate(uint32_t& sort, V& payload, const uint32_t random) {
    //TODO ASSERT ERROR
}

template <>
__device__ void SegRandomTemplate<uint32_t>(uint32_t& sort, uint32_t& payload,
...
template <>
__device__ void SegRandomTemplate<double>(uint32_t& sort, double& payload, const uint32_t random) {
    sort = random;
    uint64_t r64 = random;
    double d;
...
template <class V>
__global__ void InitFixedSegLengthRandomValue(uint32_t* sort, V* payload, uint32_t segLength,
                                              uint32_t totalSegCount, uint32_t bitsToSort,
                                              uint32_t seed) {
    const uint32_t sCount = totalSegCount;
    const uint32_t sLength = segLength;
    ...
    for (uint32_t k = blockIdx.x; k < sCount; k += gridDim.x) {
        const uint32_t devOffset = k * sLength;
        for (uint32_t i = threadIdx.x; i < sLength; i += blockDim.x) {
            SegRandomTemplate<V>(sort[i + devOffset], payload[i + devOffset],
                                 LCGTAUS(prngState) & bitMask);
        }
    }
}

template <class V>
__global__ void InitRandomSegLengthRandomValue(uint32_t* sort, V* payload, uint32_t* segments,
                                               uint32_t totalSegCount, uint32_t totalSegLength,
                                               uint32_t bitsToSort, uint32_t seed) {
    const uint32_t sCount = totalSegCount;
    const uint32_t bitMask = (uint32_t)((1ULL << bitsToSort) - 1);
    const uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    uint4 prngState = make_uint4(idx * 4 * seed, (idx * 4 + 1) * seed, (idx * 4 + 2) * seed,
    ...
    for (uint32_t k = blockIdx.x; k < sCount; k += gridDim.x) {
        ...
        for (uint32_t i = threadIdx.x; i < segLength; i += blockDim.x) {
            SegRandomTemplate<V>(sort[i + segmentStart], payload[i + segmentStart],
                                 LCGTAUS(prngState) & bitMask);
        }
    }
}
...
__global__ void InitRandomSegLengthUniqueValue(uint32_t* sort, uint32_t* payload,
                                               uint32_t* segments, uint32_t totalSegCount,
                                               uint32_t totalSegLength, uint32_t seed) {
    const uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    uint4 prngState = make_uint4(idx * 4 * seed, (idx * 4 + 1) * seed, (idx * 4 + 2) * seed,
                                 (idx * 4 + 3) * seed);
    for (uint32_t block = blockIdx.x; block < totalSegCount; block += gridDim.x) {
        ...
        const uint32_t segmentEnd =
            block + 1 == totalSegCount ? totalSegLength : segments[block + 1];
        const uint32_t segLength = segmentEnd - segmentStart;
        ...
        if (segLength <= 4096) {
            ...
            for (uint32_t t = 0; t < 2; ++t) {
                ...
                for (uint32_t i = 1; i < 3; ++i) {
                    const uint32_t part = segLength >> i;
                    for (uint32_t j = 0; j < i; ++j) {
                        for (uint32_t k = threadIdx.x; k < part; k += blockDim.x) {
                            const uint32_t rand = LCGTAUS(prngState);
                            const uint32_t index = k + j * part * 2;
                            if (rand < 0x80000000 && index < segLength) {
                                const uint32_t t = s_mem[index];
        ...
        //direct device memory here, unnecessary for now as we use this for test > 4096
        // if (segLength > 4096)
    }
}
...
__global__ void InitSegLengthsFixed(uint32_t* segments, uint32_t totalSegCount,
                                    uint32_t segmentLength) {
    const uint32_t segLength = segmentLength;
    for (uint32_t i = threadIdx.x + blockIdx.x * blockDim.x; i < totalSegCount;
         i += blockDim.x * gridDim.x)
        segments[i] = i * segLength;
}

//NOTE: this is nondeterministic, i.e. same seed will not result generation of same values.
...
__global__ void InitSegLengthsRandom(uint32_t* segments, volatile uint32_t* totalLength,
                                     uint32_t seed, uint32_t maxTotalLength,
                                     uint32_t maxSegLength) {
    const uint32_t idx = threadIdx.x + blockDim.x * blockIdx.x;
    uint4 prngState = make_uint4(idx * 4 * seed, (idx * 4 + 1) * seed, (idx * 4 + 2) * seed,
                                 (idx * 4 + 3) * seed);
    do {
        uint32_t t = LCGTAUS(prngState) % maxSegLength + 1;
...
__global__ void Validate(uint32_t* sort, uint32_t* errCount, uint32_t size) {
    for (uint32_t i = threadIdx.x + 1 + blockIdx.x * blockDim.x; i < size;
        i += blockDim.x * gridDim.x) {
        if (sort[i - 1] > sort[i]) {
...
__global__ void Validate(uint32_t* sort, uint32_t* sortPayload, uint32_t* errCount, uint32_t size) {
    for (uint32_t i = threadIdx.x + 1 + blockIdx.x * blockDim.x; i < size;
        ...
        if (sortPayload[i - 1] > sortPayload[i]) {
            atomicAdd(&errCount[0], 1);
        }
    }
}
...
//For SegSort not global . . .
__global__ void ValidateInitialOneSweepState(uint32_t* passHistograms, uint32_t* errCount,
...
template <class V>
__device__ __forceinline__ bool SegValidateTemplate(const V& x, const V& y) {
    //TODO ERROR ASSERT here
}

template <>
__device__ __forceinline__ bool SegValidateTemplate<uint32_t>(const uint32_t& x,
                                                              const uint32_t& y) {
    return x > y;
}

template <>
__device__ __forceinline__ bool SegValidateTemplate<double>(const double& x, const double& y) {
    uint32_t u1;
    uint32_t u2;
...
template <class V>
__global__ void ValidateFixLengthSegments(uint32_t* sort, V* payload, uint32_t* errCount,
                                          uint32_t segLength, uint32_t totalSegCount) {
    const uint32_t sCount = totalSegCount;
    const uint32_t sLength = segLength;
...
template <class V>
__device__ __forceinline__ bool SegValidatePrintTemplate(const V& x, const V& y,
                                                         const uint32_t segIndex,
                                                         const uint32_t segLength,
                                                         const bool isKey) {}

template <>
__device__ __forceinline__ bool SegValidatePrintTemplate<uint32_t>(const uint32_t& x,
                                                                   const uint32_t& y,
                                                                   const uint32_t segIndex,
                                                                   const uint32_t segLength,
                                                                   const bool isKey) {
...
template <>
__device__ __forceinline__ bool SegValidatePrintTemplate<double>(const double& x, const double& y,
                                                                 const uint32_t segIndex,
                                                                 const uint32_t segLength,
                                                                 const bool isKey) {
    uint32_t u1;
...
template <class V>
__global__ void ValidateRandomLengthSegments(uint32_t* sort, V* payload, uint32_t* segments,
                                             uint32_t* errCount, uint32_t totalSegLength,
                                             uint32_t totalSegCount, const bool verbose) {
...
template <class V>
__global__ void ValidateSegSortSanity(uint32_t* sortA, uint32_t* sortB, V* payloadA, V* payloadB,
                                      uint32_t* errCount, const uint32_t totalSegLength) {
...
__global__ void ValidateBinningRandomSegLengths(uint32_t* segments, uint32_t* binOffsets,
                                                uint32_t* segHist, uint32_t* packedSegCounts,
                                                uint32_t* errCount, const uint32_t totalSegCount,
                                                const uint32_t totalSegLength,
                                                const uint32_t totalBinCount, bool verbose) {
    for (uint32_t i = threadIdx.x + blockDim.x * blockIdx.x; i < totalBinCount;
         i += blockDim.x * gridDim.x) {
        ...
        if (i < segHist[1]) {
            ...
            if (endIndex > totalSegCount) {
                ...
                for (uint32_t j = 0; j < packedSegmentCount; ++j) {
                    ...
                    uint32_t end =
                        nextIndex == totalSegCount ? totalSegLength : segments[nextIndex];
