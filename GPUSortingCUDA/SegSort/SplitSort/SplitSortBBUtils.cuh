...
namespace SplitSortInternal {
    ...
    //2 keys
    template <class K>
    __device__ __forceinline__ void t2_kv2(K& key, uint32_t& index) {
        exch_intxn(key, index, 0x1, getLaneId() & 1);
    }

    //4 keys
    template <class K>
    __device__ __forceinline__ void t4_kv4(K& key, uint32_t& index) {
    ...
    //8 keys
    template <class K>
    __device__ __forceinline__ void t8_kv8(K& key, uint32_t& index) {
    ...
    //16 keys
    template <class K>
    __device__ __forceinline__ void t16_kv16(K& key, uint32_t& index) {
    ...
    template <class K>
    __device__ __forceinline__ void RegSortFallback(K& key, uint32_t& index,
    ...
    __device__ __forceinline__ uint2 BinarySearch(const uint32_t* s_warpBins,
    ...
    __device__ __forceinline__ uint32_t find_kth3(const uint2* a, const uint2* b, const int length,
                                                  const int diag) {
    ...
    __device__ __forceinline__ uint32_t find_kth3_device(const uint32_t* a, const uint32_t* b,
                                                         const int length, const int diag) {
        int begin = max(0, diag - length);
        int end = min(diag, length);
    ...
    __device__ __forceinline__ uint32_t find_kth3_device_partial(const uint32_t* a,
                                                                 const uint32_t* b,
                                                                 const int length, const int diag,
                                                                 const int topLength) {
        int begin = max(0, diag - length);
    ...
    template <class T>
    __device__ __forceinline__ void TransposeAndWrite8(T* t, T* dest, const uint32_t mergeId,
