...
namespace SplitSortInternal {
    ...
    __device__ __forceinline__ void LoadFull(const uint32_t* segments, uint32_t* threadSegments,
                                             const uint32_t partIndex) {
        #pragma unroll
        for (uint32_t i = threadIdx.x * SPT + PART_SIZE * partIndex, k = 0; k < SPT; ++i, ++k) {
            threadSegments[k] = segments[i + 1] - segments[i];
        }
    }

    template <uint32_t PART_SIZE, uint32_t SPT>
    __device__ __forceinline__ void LoadPartial(const uint32_t* segments, uint32_t* threadSegments,
                                                const uint32_t partIndex,
    ...
    template <uint32_t PART_SIZE, uint32_t SPT>
    __device__ __forceinline__ void LoadSegments(const uint32_t* segments, uint32_t* threadSegments,
                                                 const uint32_t partIndex,
                                                 const uint32_t totalSegCount,
                                                 const uint32_t totalSegLength) {
        ...
        if (gridDim.x > 1) {
            if (partIndex < gridDim.x - 1) {
                LoadFull<PART_SIZE, SPT>(segments, threadSegments, partIndex);
            } else {
                LoadPartial<PART_SIZE, SPT>(segments, threadSegments, partIndex, totalSegCount,
                                            totalSegLength);
            }
        } else {
            LoadPartial<PART_SIZE, SPT>(segments, threadSegments, partIndex, totalSegCount,
    ...
    __device__ __forceinline__ void PackBin(uint32_t* threadSegments, uint32_t* s_warpHist,
                                            uint32_t& currentBinTotal, uint32_t& currentBinCount,
                                            uint32_t& packedBinCount, const uint32_t i) {
        ...
        if (threadSegments[i] > MIN_BIN_SIZE) {
            ...
            if (currentBinCount) {
                ...
                reinterpret_cast<uint16_t*>(threadSegments)[packedBinCount * 2 + 1] =
                    i - currentBinCount;
    ...
    __device__ __forceinline__ void AttemptPackHangingBin(uint32_t* threadSegments,
                                                          uint32_t* s_warpHist,
        ...
        if (currentBinCount) {
            reinterpret_cast<uint16_t*>(threadSegments)[packedBinCount * 2] = currentBinCount;
            reinterpret_cast<uint16_t*>(threadSegments)[packedBinCount * 2 + 1] =
                threadRunLength - currentBinCount;
            atomicAdd((uint32_t*)&s_warpHist[0], 1);
    ...
    __device__ __forceinline__ void NextFitBinPackFull(uint32_t* threadSegments,
                                                       uint32_t* s_warpHist,
                                                       uint32_t& packedBinCount) {
        ...
        for (uint32_t i = 0; i < SPT; ++i) {
            PackBin<MIN_BIN_SIZE>(threadSegments, s_warpHist, currentBinTotal, currentBinCount,
                                  packedBinCount, i);
        }

        AttemptPackHangingBin(threadSegments, s_warpHist, packedBinCount, currentBinCount, SPT);
    }

    template <int32_t SPT, uint32_t MIN_BIN_SIZE>
    __device__ __forceinline__ void NextFitBinPackPartial(uint32_t* threadSegments,
                                                          uint32_t* s_warpHist,
                                                          uint32_t& packedBinCount,
        ...
        AttemptPackHangingBin(threadSegments, s_warpHist, packedBinCount, currentBinCount,
                              (uint32_t)threadRunLength);
    }

    template <uint32_t PART_SIZE, uint32_t SPT, uint32_t MIN_BIN_SIZE>
    __device__ __forceinline__ void NextFitBinPack(uint32_t* threadSegments, uint32_t* s_warpHist,
                                                   uint32_t& packedBinCount,
                                                   const uint32_t partIndex,
                                                   const uint32_t totalSegCount) {
        if (partIndex < gridDim.x - 1) {
            NextFitBinPackFull<SPT, MIN_BIN_SIZE>(threadSegments, s_warpHist, packedBinCount);
        }

        if (partIndex == gridDim.x - 1) {
            NextFitBinPackPartial<(int32_t)SPT, MIN_BIN_SIZE>(
                threadSegments, s_warpHist, packedBinCount, totalSegCount - PART_SIZE * partIndex);
        }
    }

    template <uint32_t WARPS>
    __device__ __forceinline__ uint32_t ScanAndPostStatusFlag(volatile uint32_t* reduction,
    ...
    __device__ __forceinline__ void Lookback(volatile uint32_t* reduction, uint32_t& s_broadcast,
                                             const uint32_t& partitionIndex) {
        if (!threadIdx.x) {
    ...
    __global__ void NextFitBinPacking(const uint32_t* segments, uint32_t* segHist,
                                      uint32_t* packedSegCounts, uint32_t* binOffsets,
                                      volatile uint32_t* index, volatile uint32_t* reduction,
        ...
        LoadSegments<PART_SIZE, SPT>(segments, threadSegments, partitionIndex, totalSegCount,
                                     totalSegLength);

        //Begin the bin packing
        uint32_t packedBinCount =
            0;  //How many packed bins (bins with more than one segment) has this thread processed?
        NextFitBinPack<PART_SIZE, SPT, MIN_BIN_SIZE>(threadSegments, s_warpHist, packedBinCount,
                                                     partitionIndex, totalSegCount);

        //Device wide prefix sum of packed bins count
        const uint32_t warpScan =
            ScanAndPostStatusFlag<WARPS>(reduction, s_reduction, packedBinCount, partitionIndex);

        if (partitionIndex)
            Lookback(reduction, s_broadcast, partitionIndex);

        //warp + block + device
        const uint32_t prev = (getLaneId() ? warpScan : 0) +
                              (threadIdx.x >= LANE_COUNT ? s_reduction[WARP_INDEX] : 0) +
        ...
        for (uint32_t i = 0; i < packedBinCount; ++i) {
            ...
            binOffsets[i + prev] =
                (threadSegments[i] >> 16) + PART_SIZE * partitionIndex + threadIdx.x * SPT;
        }

        //Add the histogram totals from this block for the large segment binning
        if (threadIdx.x < SEG_INFO_SIZE) {
            atomicAdd((uint32_t*)&segHist[threadIdx.x],
                      s_hist[threadIdx.x] + s_hist[threadIdx.x + SEG_INFO_SIZE]);
        }
    }
    ...
    template <uint32_t MIN_BIN_SIZE>
    __device__ __forceinline__ void Bin(const uint32_t& segLength, uint32_t* segHist,
                                        uint32_t* binOffsets, uint32_t segIndex) {
    ...
    template <uint32_t MIN_BIN_SIZE>
    __global__ void BinSimple(const uint32_t* segments, uint32_t* segHist, uint32_t* binOffsets,
                              const uint32_t totalSegCount, const uint32_t totalSegLength) {
        ...
        if (idx < totalSegCount) {
            ...
            Bin<MIN_BIN_SIZE>(segLength, segHist, binOffsets, idx);
        }
    }

    __device__ __forceinline__ void ThreadScan(uint32_t& segLength, uint32_t& reduction) {
        if (segLength <= 131072)
            segLength = 0;
    ...
    template <uint32_t PART_SIZE,     //size of a partition tile
              ...
              uint32_t WARPS,         //warps in a threadblock
              uint32_t MIN_BIN_SIZE>  //Segments below this length are packed together
    __global__ void BinAndCoalesce(const uint32_t* segments, uint32_t* segHist,
                                   uint32_t* binOffsets, uint32_t* largeSegmentOffsets,
                                   volatile uint32_t* index, volatile uint32_t* reduction,
                                   const uint32_t totalSegCount, const uint32_t totalSegLength) {
        __shared__ uint32_t s_broadcast;
        __shared__ uint32_t s_reduction[WARPS];
        ...
        if (partitionIndex < gridDim.x - 1) {
            #pragma unroll
            for (uint32_t i = threadIdx.x * SPT + partitionIndex * PART_SIZE, k = 0; k < SPT;
                 ++i, ++k) {
                Bin<MIN_BIN_SIZE>(threadSegments[k], segHist, binOffsets, i);
