...
namespace SplitSortInternal {
    ...
    __global__ void SortLe32(const uint32_t* segments, const uint32_t* binOffsets,
                             const uint32_t* minBinSegCounts, uint32_t* sort, V* values,
                             const uint32_t totalSegCount, const uint32_t totalSegLength,
                             const uint32_t segCountInBin) {
        SplitSortBins32<32, 128, 4, BITS_TO_SORT, V>(segments, binOffsets, minBinSegCounts, sort,
                                                     values, totalSegCount, totalSegLength,
                                                     segCountInBin);
    };

    //w4_t32_kv64_cute64_wMerge
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt32Le64(const uint32_t* segments, const uint32_t* binOffsets,
                                 uint32_t* sort, V* values, const uint32_t totalSegCount,
                                 const uint32_t totalSegLength, const uint32_t segCountInBin) {
        SplitSortWarp<2, 64, 256, 4, 5, 6, V>(segments, binOffsets, sort, values, totalSegCount,
                                              totalSegLength, segCountInBin,
                                              CuteSort32<BITS_TO_SORT, 2>);
    }

    //w2_t32_kv128_cute64_wMerge
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt64Le128(const uint32_t* segments, const uint32_t* binOffsets,
                                  uint32_t* sort, V* values, const uint32_t totalSegCount,
                                  const uint32_t totalSegLength, const uint32_t segCountInBin) {
        SplitSortWarp<4, 128, 256, 2, 6, 7, V>(segments, binOffsets, sort, values, totalSegCount,
                                               totalSegLength, segCountInBin,
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt128Le256(const uint32_t* segments, const uint32_t* binOffsets,
                                   uint32_t* sort, V* values, const uint32_t totalSegCount,
                                   const uint32_t totalSegLength) {
        if constexpr (BITS_TO_SORT > 24) {
            SplitSortBlock<4, 128, 256, 2, 6, 7, 8, false, V>(segments, binOffsets, sort, values,
                                                              totalSegCount, totalSegLength,
                                                              CuteSort64<BITS_TO_SORT, 4>);
        } else {
            SplitSortBlock<4, 128, 256, 2, 7, 7, 8, false, V>(segments, binOffsets, sort, values,
                                                              totalSegCount, totalSegLength,
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt256Le512(const uint32_t* segments, const uint32_t* binOffsets,
                                   uint32_t* sort, V* values, const uint32_t totalSegCount,
                                   const uint32_t totalSegLength) {
        SplitSortBlock<4, 128, 512, 4, 7, 7, 9, false, V>(segments, binOffsets, sort, values,
                                                          totalSegCount, totalSegLength,
                                                          CuteSort128<BITS_TO_SORT, 4>);
    }
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt512Le1024(const uint32_t* segments, const uint32_t* binOffsets,
                                    uint32_t* sort, V* values, const uint32_t totalSegCount,
                                    const uint32_t totalSegLength) {
        if constexpr (BITS_TO_SORT > 24) {
            SplitSortBlock<4, 128, 1024, 8, 7, 7, 10, false, V>(segments, binOffsets, sort, values,
                                                                totalSegCount, totalSegLength,
                                                                CuteSort128<BITS_TO_SORT, 4>);
        } else {
            __shared__ uint32_t s_mem[256 * 4 + 1024];  //RADIX * WARPS + PART_SIZE
            uint32_t totalLocalLength;
            GetSegmentInfoRadixFine(segments, binOffsets, sort, values, totalSegCount,
                                    totalSegLength, totalLocalLength);

            if (totalLocalLength <= 640) {
                SplitSortRadixFine<4, 5, 160, 640, ROUND_UP_BITS_TO_SORT, 256, 255, 8, V>(
                    s_mem,
                    &s_mem[1024],  //RADIX * WARPS
                    sort, values, totalLocalLength);
            }
    ...
    __global__ void __launch_bounds__(256)
        SortGt1024Le2048(const uint32_t* segments, const uint32_t* binOffsets, uint32_t* sort,
                         V* values, const uint32_t totalSegCount, const uint32_t totalSegLength) {
        __shared__ uint32_t s_mem[256 * 8 + 2048];  //RADIX * WARPS + PART_SIZE
        uint32_t totalLocalLength;
        GetSegmentInfoRadixFine(segments, binOffsets, sort, values, totalSegCount, totalSegLength,
                                totalLocalLength);
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt2048Le4096(const uint32_t* segments, const uint32_t* binOffsets,
                                     uint32_t* sort, V* values, const uint32_t totalSegCount,
                                     const uint32_t totalSegLength) {
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt4096Le6144(const uint32_t* segments, const uint32_t* binOffsets,
                                     uint32_t* sort, V* values, const uint32_t totalSegCount,
                                     const uint32_t totalSegLength) {
    ...
    template <uint32_t BITS_TO_SORT, class V>
    __global__ void SortGt6144Le8192(const uint32_t* segments, const uint32_t* binOffsets,
                                     uint32_t* sort, V* values, const uint32_t totalSegCount,
                                     const uint32_t totalSegLength) {
    ...
    __global__ void __launch_bounds__(512)
        SortGt8192(const uint32_t* segments, const uint32_t* binOffsets, uint32_t* sort, V* values,
                   const uint32_t totalSegCount, const uint32_t totalSegLength) {
        extern __shared__ uint32_t s_mem[];
        uint32_t totalLocalLength;
        GetSegmentInfoRadixMerge<V, GRID_STRIDE_LOG, (1 << GRID_STRIDE_LOG) - 1, 8192>(
            segments, binOffsets, sort, values, totalSegCount, totalSegLength, totalLocalLength);
    ...
    __global__ void __launch_bounds__(1024)
        MergeGt8192(const uint32_t* segments, const uint32_t* binOffsets, uint32_t* sort, V* values,
                    volatile uint32_t* index, volatile uint32_t* gridLock,
                    const uint32_t totalSegCount, const uint32_t totalSegLength) {
        SplitSortMergeDeviceGrid<V, 8, GRID_STRIDE_LOG, (1 << GRID_STRIDE_LOG) - 1, START_LOG,
                                 END_LOG, 10>(segments, binOffsets, sort, values, index, gridLock,
                                              totalSegCount, totalSegLength);
    }

    template <class V, uint32_t BITS_TO_SORT, uint32_t START_LOG, uint32_t END_LOG,
              uint32_t GRID_STRIDE_LOG>
    __host__ void DispatchGt8192(const uint32_t* segments, const uint32_t* binOffsets,
                                 uint32_t* sort, V* values, const uint32_t totalSegCount,
                                 const uint32_t totalSegLength, const uint32_t segsInCurBin) {
        cudaFuncSetAttribute(SplitSortInternal::SortGt8192<BITS_TO_SORT, GRID_STRIDE_LOG, V>,
                             cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
        SplitSortInternal::SortGt8192<BITS_TO_SORT, GRID_STRIDE_LOG, V>
            <<<(segsInCurBin << GRID_STRIDE_LOG), 512, 65536, 0>>>(
                segments, binOffsets, sort, values, totalSegCount, totalSegLength);

        uint32_t* gridLock;
        cudaMalloc(&gridLock, (segsInCurBin * 2 + 1) * sizeof(uint32_t));
        cudaMemset(gridLock, 0, (segsInCurBin * 2 + 1) * sizeof(uint32_t));

        SplitSortInternal::MergeGt8192<V, START_LOG, END_LOG, GRID_STRIDE_LOG>
            <<<(segsInCurBin << GRID_STRIDE_LOG), 1024>>>(segments, binOffsets, sort, values,
                                                          gridLock, gridLock + 1, totalSegCount,
                                                          totalSegLength);
    ...
    __host__ __forceinline__ uint32_t* GetBinAndCoalesceReductionsPointer(
        void* tempMem, const uint32_t nextFitPartitions) {
        return &((uint32_t*)tempMem)[2 + SEG_INFO_SIZE + nextFitPartitions];
    }

    __host__ __forceinline__ uint32_t* GetPackedSegCountsPointer(void* tempMem,
                                                                 const uint32_t nextFitPartitions) {
        return &((uint32_t*)tempMem)[2 + SEG_INFO_SIZE + nextFitPartitions * 2];
    }

    __host__ __forceinline__ uint32_t* GetBinOffsetsPointer(void* tempMem,
                                                            const uint32_t nextFitPartitions,
                                                            const uint32_t totalSegCount) {
        return &((uint32_t*)tempMem)[2 + SEG_INFO_SIZE + nextFitPartitions * 2 + totalSegCount];
    }

    __host__ __forceinline__ uint32_t* GetLargeSegmentOffsetsPointer(
        void* tempMem, const uint32_t nextFitPartitions, const uint32_t totalSegCount) {
    ...
    //***********************************************************************
    __host__ void SplitSortBinning(uint32_t* segments, uint32_t* binOffsets,
                                   uint32_t* packedSegCounts, void* tempMem, uint32_t* segInfo,
                                   const uint32_t totalSegCount, const uint32_t totalSegLength,
                                   const uint32_t nextFitPartitions) {
        ...
        NextFitBinPacking<NEXT_FIT_PART_SIZE, k_nextFitBlockDim / LANE_COUNT, k_nextFitSPT,
                          k_minBinSize, SEG_INFO_SIZE><<<nextFitPartitions, k_nextFitBlockDim>>>(
            segments, segHistDevicePointer, packedSegCounts, binOffsets,
            GetAtomicIndexPointer(tempMem), GetNextFitReductionPointer(tempMem), totalSegCount,
            totalSegLength);

        BinningScan<SEG_HIST_SIZE><<<1, 32>>>(segHistDevicePointer);

        cudaMemcpyAsync(segInfo, GetSegHistDevicePointer(tempMem), SEG_INFO_SIZE * sizeof(uint32_t),
        ...
        if (segInfo[0] - segInfo[SEG_INFO_SIZE - 2] == 0) {
            ...
            BinSimple<k_minBinSize><<<dvrup<k_binThreads>(totalSegCount), k_binThreads>>>(
                segments, segHistDevicePointer, binOffsets, totalSegCount, totalSegLength);
        }
    }
}  // namespace SplitSortInternal
...
//TEMPORARY MEMORY
//***********************************************************************
__host__ void SplitSortAllocateTempMemory(const uint32_t totalSegLength,
                                          const uint32_t totalSegCount, void*& tempMem) {
...
__host__ void SplitSortPairs(uint32_t* segments, uint32_t* sort, V* values,
    ...
    uint32_t* packedSegCounts =
        SplitSortInternal::GetPackedSegCountsPointer(tempMem, nextFitPartitions);
    ...
    if (segInfo[0] - segInfo[SEG_INFO_SIZE - 2] > 0) {
        ...
        if (segsInCurBin) {
            SplitSortInternal::SortLe32<BITS_TO_SORT>
                <<<SplitSortInternal::dvrup<4>(segsInCurBin), 128>>>(
                    segments, binOffsets, packedSegCounts, sort, values, totalSegCount,
                    totalSegLength, segsInCurBin);
        }

        segsInCurBin = segInfo[2] - segInfo[1];
        if (segsInCurBin) {
            SplitSortInternal::SortGt32Le64<BITS_TO_SORT>
                <<<SplitSortInternal::dvrup<4>(segsInCurBin), 128>>>(
                    segments, binOffsets + segInfo[1], sort, values, totalSegCount, totalSegLength,
                    segsInCurBin);
        }

        segsInCurBin = segInfo[3] - segInfo[2];
        if (segsInCurBin) {
            SplitSortInternal::SortGt64Le128<BITS_TO_SORT>
                <<<SplitSortInternal::dvrup<2>(segsInCurBin), 64>>>(
                    segments, binOffsets + segInfo[2], sort, values, totalSegCount, totalSegLength,
                    segsInCurBin);
        }
        ...
        if (segsInCurBin) {
            SplitSortInternal::SortGt128Le256<BITS_TO_SORT><<<segsInCurBin, 64>>>(
                segments, binOffsets + segInfo[3], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[5] - segInfo[4];
        if (segsInCurBin) {
            SplitSortInternal::SortGt256Le512<BITS_TO_SORT><<<segsInCurBin, 128>>>(
                segments, binOffsets + segInfo[4], sort, values, totalSegCount, totalSegLength);
        }
        ...
        if (segsInCurBin) {
            ...
            SplitSortInternal::SortGt512Le1024<BITS_TO_SORT><<<segsInCurBin, threads>>>(
                segments, binOffsets + segInfo[5], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[7] - segInfo[6];
        if (segsInCurBin) {
            SplitSortInternal::SortGt1024Le2048<BITS_TO_SORT><<<segsInCurBin, 256>>>(
                segments, binOffsets + segInfo[6], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[8] - segInfo[7];
        if (segsInCurBin) {
            SplitSortInternal::SortGt2048Le4096<BITS_TO_SORT><<<segsInCurBin, 512>>>(
                segments, binOffsets + segInfo[7], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[9] - segInfo[8];
        if (segsInCurBin) {
            SplitSortInternal::SortGt4096Le6144<BITS_TO_SORT><<<segsInCurBin, 512>>>(
                segments, binOffsets + segInfo[8], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[10] - segInfo[9];
        if (segsInCurBin) {
            cudaFuncSetAttribute(SplitSortInternal::SortGt6144Le8192<BITS_TO_SORT, V>,
                                 cudaFuncAttributeMaxDynamicSharedMemorySize, 65536);
            SplitSortInternal::SortGt6144Le8192<BITS_TO_SORT, V><<<segsInCurBin, 512, 65536, 0>>>(
                segments, binOffsets + segInfo[9], sort, values, totalSegCount, totalSegLength);
        }

        segsInCurBin = segInfo[11] - segInfo[10];
        if (segsInCurBin) {
            SplitSortInternal::DispatchGt8192<V, BITS_TO_SORT, 13, 14, 1>(
                segments, binOffsets + segInfo[10], sort, values, totalSegCount, totalSegLength,
                segsInCurBin);
        }
        ...
        if (segsInCurBin) {
            SplitSortInternal::DispatchGt8192<V, BITS_TO_SORT, 13, 15, 2>(
                segments, binOffsets + segInfo[11], sort, values, totalSegCount, totalSegLength,
                segsInCurBin);
        }
        ...
        if (segsInCurBin) {
            SplitSortInternal::DispatchGt8192<V, BITS_TO_SORT, 13, 16, 3>(
                segments, binOffsets + segInfo[12], sort, values, totalSegCount, totalSegLength,
                segsInCurBin);
        }
        ...
        if (segsInCurBin) {
            SplitSortInternal::DispatchGt8192<V, BITS_TO_SORT, 13, 17, 4>(
                segments, binOffsets + segInfo[13], sort, values, totalSegCount, totalSegLength,
                segsInCurBin);
        }
