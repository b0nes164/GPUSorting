...
namespace SplitSortInternal {
    ...
    __device__ __forceinline__ void count(const uint8_t* key8, uint32_t* s_hist0, uint32_t* s_hist1,
                                          uint32_t* s_hist2, uint32_t* s_hist3) {
        //Will never happen
        //TODO error assertion here or something to be safe ?
    }

    template <>
    __device__ __forceinline__ void count<1>(const uint8_t* key8, uint32_t* s_warpsHist0,
                                             uint32_t* s_warpsHist1, uint32_t* s_warpsHist2,
                                             uint32_t* s_warpsHist3) {
        atomicAdd(&s_warpsHist0[key8[0]], 1);
    }

    template <>
    __device__ __forceinline__ void count<2>(const uint8_t* key8, uint32_t* s_warpsHist0,
                                             uint32_t* s_warpsHist1, uint32_t* s_warpsHist2,
                                             uint32_t* s_warpsHist3) {
        atomicAdd(&s_warpsHist0[key8[0]], 1);
    ...
    template <>
    __device__ __forceinline__ void count<3>(const uint8_t* key8, uint32_t* s_warpsHist0,
                                             uint32_t* s_warpsHist1, uint32_t* s_warpsHist2,
                                             uint32_t* s_warpsHist3) {
        atomicAdd(&s_warpsHist0[key8[0]], 1);
        atomicAdd(&s_warpsHist1[key8[1]], 1);
    ...
    template <>
    __device__ __forceinline__ void count<4>(const uint8_t* key8, uint32_t* s_warpsHist0,
                                             uint32_t* s_warpsHist1, uint32_t* s_warpsHist2,
                                             uint32_t* s_warpsHist3) {
        atomicAdd(&s_warpsHist0[key8[0]], 1);
        atomicAdd(&s_warpsHist1[key8[1]], 1);
    ...
    __device__ __forceinline__ void ReduceAndAddDigitCounts(uint32_t* globalHistogram,
                                                            uint32_t* s_hist0, uint32_t* s_hist1,
                                                            uint32_t* s_hist2, uint32_t* s_hist3) {
    ...
    __device__ __forceinline__ void AddSegIdDigitCounts(uint32_t* globalHistogram,
        ...
        if (!threadIdx.x) {
            ...
            atomicAdd((uint32_t*)&globalHistogram[segId8[0] + RADIX_OFFSET_FIFTH],
                      totalLocalLength);
    ...
    template <class F, uint32_t WARPS, uint32_t BITS_TO_SORT, uint32_t RADIX_PASSES>
    __global__ void GlobalHistAndFixInPlace(const uint32_t* segments, uint32_t* sort, F* fixA,
                                            uint32_t* globalHistogram, const uint32_t totalSegCount,
        ...
        const uint32_t segmentEnd =
            blockIdx.x + 1 == totalSegCount ? totalSegLength : segments[blockIdx.x + 1];
        ...
        if constexpr (RADIX_PASSES <= 4) {
            for (uint32_t i = threadIdx.x; i < totalLocalLength; i += blockDim.x) {
                ...
                if constexpr (BITS_TO_SORT <
                              32)  //Dumb hack to supress compiler warning. This is already checked
                    key[0] |= blockIdx.x << BITS_TO_SORT;
                count<RADIX_PASSES>(reinterpret_cast<uint8_t*>(key), s_warpsHist0, s_warpsHist1,
                                    s_warpsHist2, s_warpsHist3);
                sort[i] = key[0];
            }
        } else {
            for (uint32_t i = threadIdx.x; i < totalLocalLength; i += blockDim.x) {
                uint32_t key[1] = {sort[i]};
                count<4>(reinterpret_cast<uint8_t*>(key), s_warpsHist0, s_warpsHist1, s_warpsHist2,
                         s_warpsHist3);
                fixA[i] = (F)blockIdx.x;
            }
        }
        ...
        ReduceAndAddDigitCounts(globalHistogram, s_hist0, s_hist1, s_hist2, s_hist3);

        //if the number of radix passes is greater than 4
        //then the segId's need to be histogrammed seperately.
        if constexpr (RADIX_PASSES > 4) {
            AddSegIdDigitCounts<sizeof(F)>(globalHistogram, blockIdx.x, totalLocalLength);
        }
    }
    ...
    template <class V, class F, uint32_t WARPS, uint32_t BITS_TO_SORT, uint32_t RADIX_PASSES>
    __global__ void GlobalHistAndFix(const uint32_t* segments, const uint32_t* binOffsets,
                                     const uint32_t* largeSegmentOffsets, const uint32_t* sort,
                                     const V* values, uint32_t* altA, V* altPayloadA, F* fixA,
                                     uint32_t* globalHistogram, const uint32_t totalSegCount,
                                     const uint32_t totalSegLength, const uint32_t size) {
        constexpr uint32_t sMemSize = WARPS / 2 * RADIX;
        __shared__ uint32_t s_hist0[sMemSize];
        __shared__ uint32_t s_hist1[sMemSize];
        __shared__ uint32_t s_hist2[sMemSize];
        ...
        //then the segId's need to be histogrammed seperately.
        if constexpr (RADIX_PASSES > 4) {
            AddSegIdDigitCounts<sizeof(F)>(globalHistogram, binOffset, totalLocalLength);
        }
    }

    __global__ void LargeScan(uint32_t* globalHistogram, uint32_t* passHistograms,
                              const uint32_t passPartitions) {
        __shared__ uint32_t s_scan[RADIX >> LANE_LOG];

        uint32_t scan =
            InclusiveWarpScanCircularShift(globalHistogram[threadIdx.x + blockIdx.x * RADIX]);
        if (!getLaneId()) {
    ...
    template <class K, class V, class F, uint32_t RADIX_SHIFT, uint32_t BITS_TO_SORT_MASK,
              bool SHOULD_SCATTER_KEYS, bool IS_NOT_PACKED, bool SHOULD_MASK>
    __global__ void __launch_bounds__(PASS_WARPS* LANE_COUNT)
        DigitBinningPass(const K* altA, const V* altPayloadA, const F* fixA, K* altB,
                         V* altPayloadB, F* fixB, volatile uint32_t* passHistogram,
        ...
        if (partitionIndex < gridDim.x - 1) {
            #pragma unroll
            for (uint32_t i = LoadOffset(partitionIndex), k = 0; k < PASS_KPT;
                 i += LANE_COUNT, ++k) {
        ...
        RankKeys<PASS_KPT, RADIX_LOG, RADIX_MASK>(keys, offsets, s_warpHist, RADIX_SHIFT);
        __syncthreads();
    ...
    __global__ void ScatterBack(const uint32_t* segments, const uint32_t* largeSegmentOffsets,
                                const uint32_t* alt, const V* altPayloads, const F* fix,
                                uint32_t* sort, V* values, const uint32_t size,
                                const uint32_t totalSegLength) {
        ...
        if (blockIdx.x < gridDim.x - 1) {
            #pragma unroll
            for (uint32_t i = threadIdx.x + blockIdx.x * PART_SIZE, k = 0; k < KEYS_PER_THREAD;
                 i += blockDim.x, ++k) {
    ...
    __host__ void DispatchGlobalHistAndFixInPlace(const uint32_t* segments, uint32_t* sort,
                                                  uint32_t* fix, uint32_t* globalHistogram,
                                                  const uint32_t totalSegCount,
                                                  const uint32_t totalSegLength,
                                                  const uint32_t radixPasses) {
        switch (radixPasses) {
            case 1:
                GlobalHistAndFixInPlace<uint32_t, 4, BITS_TO_SORT, 1>
                    <<<totalSegCount, 128>>>(segments, sort,
                                             fix,  //Dummy
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 2:
                GlobalHistAndFixInPlace<uint32_t, 4, BITS_TO_SORT, 2>
                    <<<totalSegCount, 128>>>(segments, sort,
                                             fix,  //Dummy
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 3:
                GlobalHistAndFixInPlace<uint32_t, 4, BITS_TO_SORT, 3>
                    <<<totalSegCount, 128>>>(segments, sort,
                                             fix,  //Dummy
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 4:
                GlobalHistAndFixInPlace<uint32_t, 4, BITS_TO_SORT, 4>
                    <<<totalSegCount, 128>>>(segments, sort,
                                             fix,  //Dummy
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 5:
                GlobalHistAndFixInPlace<uint8_t, 4, BITS_TO_SORT, 5>
                    <<<totalSegCount, 128>>>(segments, sort, reinterpret_cast<uint8_t*>(fix),
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 6:
                GlobalHistAndFixInPlace<uint16_t, 4, BITS_TO_SORT, 6>
                    <<<totalSegCount, 128>>>(segments, sort, reinterpret_cast<uint16_t*>(fix),
                                             globalHistogram, totalSegCount, totalSegLength);
                break;
            case 7:
    ...
    template <class V, uint32_t BITS_TO_SORT>
    __host__ void DispatchDigitBinningPasses(uint32_t* altA, V* altPayloadA, uint32_t* altB,
                                             V* altPayloadB, uint32_t* fix, uint32_t* indexes,
                                             uint32_t* passHists, const uint32_t radixPasses,
        ...
        switch (radixPasses) {
            case 1: {
                cudaFuncSetAttribute(
                    ...
                    cudaFuncAttributeMaxDynamicSharedMemorySize, k_smemBytes);
                DigitBinningPass<uint32_t, V, uint32_t, 0, k_bitsToSortMask, true, false, true>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altA, altPayloadA, fix, altB, altPayloadB, fix, passHists, indexes,
                        totalSegLength);
                break;
            }
            case 2: {
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 0, k_bitsToSortMask,
                                                      true, false, false>,
                                     cudaFuncAttributeMaxDynamicSharedMemorySize, k_smemBytes);
                ...
                DigitBinningPass<uint32_t, V, uint32_t, 8, k_bitsToSortMask, true, false, true>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altB, altPayloadB, fix, altA, altPayloadA, fix,
                        &passHists[binningPartitions * RADIX], &indexes[1], totalSegLength);
                break;
            }
            case 3: {
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 0, k_bitsToSortMask,
                ...
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 8, k_bitsToSortMask,
                                                      true, false, false>,
                                     cudaFuncAttributeMaxDynamicSharedMemorySize, k_smemBytes);
                ...
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 16, k_bitsToSortMask,
                                                      true, false, true>,
                                     cudaFuncAttributeMaxDynamicSharedMemorySize, k_smemBytes);
                DigitBinningPass<uint32_t, V, uint32_t, 16, k_bitsToSortMask, true, false, true>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altA, altPayloadA, fix, altB, altPayloadB, fix,
                        &passHists[binningPartitions * RADIX * 2], &indexes[2], totalSegLength);
                break;
            }
            case 4: {
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 0, k_bitsToSortMask,
                ...
                cudaFuncSetAttribute(DigitBinningPass<uint32_t, V, uint32_t, 24, k_bitsToSortMask,
                                                      true, false, true>,
                                     cudaFuncAttributeMaxDynamicSharedMemorySize, k_smemBytes);
                DigitBinningPass<uint32_t, V, uint32_t, 24, k_bitsToSortMask, true, false, true>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altB, altPayloadB, fix, altA, altPayloadA, fix,
                        &passHists[binningPartitions * RADIX * 3], &indexes[3], totalSegLength);
                break;
            }
            case 5: {
                ...
                DigitBinningPass<uint32_t, V, uint8_t, 0, 0, true, true, false>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altA, altPayloadA, fixA, altB, altPayloadB, fixB, passHists, indexes,
                        totalSegLength);
                ...
                DigitBinningPass<uint32_t, V, uint8_t, 8, 0, true, true, false>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altB, altPayloadB, fixB, altA, altPayloadA, fixA,
                        &passHists[binningPartitions * RADIX], &indexes[1], totalSegLength);
                ...
                DigitBinningPass<uint32_t, V, uint8_t, 16, 0, true, true, false>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        altA, altPayloadA, fixA, altB, altPayloadB, fixB,
                        &passHists[binningPartitions * RADIX * 2], &indexes[2], totalSegLength);
                ...
                DigitBinningPass<uint8_t, V, uint32_t, 0, 0, false, true, false>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        fixA, altPayloadA, altA, fixB, altPayloadB, altB,
                        &passHists[binningPartitions * RADIX * 4], &indexes[4], totalSegLength);
                break;
            }
            case 6: {
                ...
                DigitBinningPass<uint16_t, V, uint32_t, 8, 0, false, true, false>
                    <<<binningPartitions, PASS_WARPS * LANE_COUNT, k_smemBytes, 0>>>(
                        fixB, altPayloadB, altB, fixA, altPayloadA, altA,
                        &passHists[binningPartitions * RADIX * 5], &indexes[5], totalSegLength);
                break;
            }
            case 7: {
                //impossible with max total length of 2^30 imposed by chained scan
    ...
    template <class V, uint32_t BITS_TO_SORT>
    __host__ void SplitSortLargeInPlace(const uint32_t* segments, uint32_t* sort, V* values,
                                        const uint32_t totalSegCount,
                                        const uint32_t totalSegLength) {
        ...
        constexpr uint32_t k_partBytes =
            k_partitionSize * sizeof(V);  //We always assume sizeof(key) <= sizeof(V)
        constexpr uint32_t k_smemBytes =
            (k_partBytes > k_histBytes ? k_partBytes : k_histBytes) + sizeof(uint32_t) * RADIX;

        const uint32_t segIdBits = findHighestBit(totalSegCount) + 1;
        const uint32_t totalBits = segIdBits + BITS_TO_SORT;
        const uint32_t radixPasses =
            totalBits <= 32 ? dvrup<RADIX_LOG>(totalBits) : 4 + dvrup<RADIX_LOG>(segIdBits);
        ...
        //Clear
        cudaMemset(indexes, 0,
                   (radixPasses * (RADIX + 1)) +
                       (binningPartitions * radixPasses * RADIX) * sizeof(uint32_t));

        DispatchGlobalHistAndFixInPlace<V, BITS_TO_SORT>(
            segments, sort, fix, globalHist, totalSegCount, totalSegLength, radixPasses);

        LargeScan<<<radixPasses, RADIX>>>(globalHist, passHists, binningPartitions);

        DispatchDigitBinningPasses<V, BITS_TO_SORT>(sort, values, alt, altPayload, fix, indexes,
                                                    passHists, radixPasses, binningPartitions,
                                                    totalSegLength, k_smemBytes);
    ...
    __host__ void DispatchGlobalHistAndFix(
        const uint32_t* segments, const uint32_t* binOffsets, const uint32_t* largeSegmentOffsets,
        uint32_t* sort, V* values, uint32_t* altA, V* altPayloadA, uint32_t* fixA,
        uint32_t* globalHistogram, const uint32_t totalSegCount, const uint32_t totalSegLength,
        const uint32_t segsInCurBin, const uint32_t radixPasses, const uint32_t size) {
        switch (radixPasses) {
            case 1:
                GlobalHistAndFix<V, uint32_t, 4, BITS_TO_SORT, 1><<<segsInCurBin, 128>>>(
                    segments, binOffsets, largeSegmentOffsets, sort, values, altA, altPayloadA,
                    fixA, globalHistogram, totalSegCount, totalSegLength, size);
                break;
            case 2:
            ...
            case 4:
                ...
                break;
            case 5:
                GlobalHistAndFix<V, uint8_t, 4, BITS_TO_SORT, 5>
                    <<<segsInCurBin, 128>>>(segments, binOffsets, largeSegmentOffsets, sort, values,
                                            altA, altPayloadA, reinterpret_cast<uint8_t*>(fixA),
                                            globalHistogram, totalSegCount, totalSegLength, size);
                break;
            case 6:
                GlobalHistAndFix<V, uint16_t, 4, BITS_TO_SORT, 6>
                    <<<segsInCurBin, 128>>>(segments, binOffsets, largeSegmentOffsets, sort, values,
                                            altA, altPayloadA, reinterpret_cast<uint16_t*>(fixA),
                                            globalHistogram, totalSegCount, totalSegLength, size);
                break;
            case 7:
    ...
    //THIS IS CURRENTLY UNUSED AND INCOMPLETE
    template <class V, uint32_t BITS_TO_SORT>
    __host__ void SplitSortLarge(const uint32_t* segments, const uint32_t* binOffsets,
                                 const uint32_t* largeSegmentOffsets, uint32_t* sort, V* values,
                                 const uint32_t totalSegCount, const uint32_t totalSegLength,
                                 const uint32_t segsInCurBin, const uint32_t size) {
