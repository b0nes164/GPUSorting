...
class SplitSortTests {
   ...
   public:
    template <class K>
    SplitSortTests(uint32_t maxTotalLength, uint32_t maxTotalSegCount, K dummy)
        : k_maxTotalLength(maxTotalLength), k_maxTotalSegCount(maxTotalSegCount) {
    ...
    void TestBinningRandomSegLength(const uint32_t testCount, const uint32_t maxSegLength,
                                    const uint32_t totalSegLength, const bool verbose) {
        if (k_maxTotalLength < (1 << 27)) {
            printf(
                "Error TestBinningRandomSegLength requires at least 1 << 27 allocated sort "
                "memory.\n");
            return;
        }
        ...
        if (maxSegLength > 65536) {
            printf(
                "Warning, currently a segment length greater than 65536 will skip binning, "
                "exiting.\n");
            return;
        }
        ...
        for (uint32_t i = 0; i < testCount; ++i) {
            ...
            DispatchInitSegmentsRandomLengthRandomValue(segInitInfo, maxSegLength, totalSegLength,
                                                        32, i + 10);
            cudaDeviceSynchronize();

            uint32_t segInfo[SEG_INFO_SIZE];
            const uint32_t nextFitPartitions =
                SplitSortInternal::GetNextFitPartitions(segInitInfo[1]);
            SplitSortInternal::SplitSortBinning(
                m_segments,
                SplitSortInternal::GetBinOffsetsPointer(m_tempMem, nextFitPartitions,
                                                        segInitInfo[1]),
                SplitSortInternal::GetPackedSegCountsPointer(m_tempMem, nextFitPartitions),
                m_tempMem, segInfo, segInitInfo[1], segInitInfo[0], nextFitPartitions);
            cudaMemcpy(m_segInfoValidate, segInfo, SEG_INFO_SIZE * sizeof(uint32_t),
                       cudaMemcpyHostToDevice);
            bool passed = ValidateBinning(segInitInfo[1], segInitInfo[0], segInfo[0],
                                          true);  //Enable for super verbose
            if (passed)
                testsPassed++;
    ...
    void FastTestRandomSegmentLengths(uint32_t testsPerSegmentLength, uint32_t minLogTwo,
                                      uint32_t maxLogTwo, uint32_t totalSegLength, bool verbose) {
        ...
        uint32_t testsPassed = 0;
        for (uint32_t maxSegLengthLog = minLogTwo; maxSegLengthLog <= maxLogTwo;
             ++maxSegLengthLog) {
            for (uint32_t i = 0; i < testsPerSegmentLength; ++i) {
                //Init
                uint32_t segInitInfo[2];
                if (DispatchInitSegmentsRandomLengthRandomValue(
                        segInitInfo, 1 << maxSegLengthLog, totalSegLength, BITS_TO_SORT, i + 10)) {
                    return;
                }
                cudaDeviceSynchronize();

                SplitSortPairs<BITS_TO_SORT>(m_segments, m_sort, m_payloads, segInitInfo[1],
                                             segInitInfo[0], m_tempMem);

                bool passed = ValidateSegSortRandomLength(segInitInfo[1], segInitInfo[0],
                                                          false);  //enable for super verbose
                if (passed)
                    testsPassed++;
    ...
    template <uint32_t BITS_TO_SORT>
    void FullTestRandomSegmentLengths(uint32_t testsPerSegmentLength, uint32_t minLogTwo,
                                      uint32_t maxLogTwo, uint32_t totalSegLength, bool verbose) {
        if (k_maxTotalSegCount < (totalSegLength >> minLogTwo))  //Too many segments
        {
            printf("Error, allocate more memory\n");
        ...
        for (uint32_t maxSegLengthLog = minLogTwo; maxSegLengthLog <= maxLogTwo;
            ...
            for (uint32_t i = 0; i < testsPerSegmentLength; ++i) {
                ...
                cudaMemcpy(sortCopy, m_sort, segInitInfo[0] * sizeof(uint32_t),
                           cudaMemcpyDeviceToDevice);
                cudaMemcpy(payloadCopy, m_payloads, segInitInfo[0] * sizeof(K),
                           cudaMemcpyDeviceToDevice);
                ...
                cub::DeviceSegmentedSort::SortPairs(
                    d_temp_storage, temp_storage_bytes, sortCopy, sortCopy, payloadCopy,
                    payloadCopy, segInitInfo[0], segInitInfo[1], m_segments, m_segments + 1);
    ...
    template <uint32_t BITS_TO_SORT>
    void FastTestFixedSegmentLength(uint32_t testsToRun, uint32_t segmentLength,
                                    uint32_t segmentCount, bool verbose) {
        ...
        printf(
            "Beginning Fast Fixed Length Test: BitsToSort: %u SegmentCount: %u SegmentLength: %u "
            "TotalSegmentLength: %u \n",
            BITS_TO_SORT, segmentCount, segmentLength, segmentCount * segmentLength);
        uint32_t testsPassed = 0;
        for (uint32_t i = 0; i < testsToRun; ++i) {
            InitSegLengthsFixed<<<256, 256>>>(m_segments, segmentCount, segmentLength);
            InitFixedSegLengthRandomValue<<<4096, 64>>>(m_sort, m_payloads, segmentLength,
                                                        segmentCount, BITS_TO_SORT, i + 10);
            cudaDeviceSynchronize();

            SplitSortPairs<BITS_TO_SORT>(m_segments, m_sort, m_payloads, segmentCount,
                                         segmentLength * segmentCount, m_tempMem);

            bool passed = ValidateSegSortRandomLength(segmentCount, segmentCount * segmentLength,
                                                      false);  //enable for super verbose
            if (passed)
                testsPassed++;
    ...
    template <uint32_t BITS_TO_SORT>
    void FullTestFixedSegmentLength(uint32_t testsToRun, uint32_t segmentLength,
                                    uint32_t segmentCount, bool verbose) {
        if (k_maxTotalLength < segmentLength * segmentCount ||
            k_maxTotalSegCount < segmentCount + 1) {
            printf("Error allocate more memory\n");
            return;
        }
        ...
        printf(
            "Beginning Full Fixed Length Test: BitsToSort: %u SegmentCount: %u SegmentLength: %u "
            "TotalSegmentLength: %u \n",
            BITS_TO_SORT, segmentCount, segmentLength, totalSegLength);

        uint32_t testsPassed = 0;
        for (uint32_t i = 0; i < testsToRun; ++i) {
            InitSegLengthsFixed<<<256, 256>>>(m_segments,
                                              segmentCount + 1,  //For CUB
                                              segmentLength);
            InitFixedSegLengthRandomValue<<<4096, 64>>>(m_sort, m_payloads, segmentLength,
                                                        segmentCount, BITS_TO_SORT, i + 10);

            cudaMemcpy(sortCopy, m_sort, totalSegLength * sizeof(uint32_t),
                       cudaMemcpyDeviceToDevice);
            cudaMemcpy(payloadCopy, m_payloads, totalSegLength * sizeof(K),
                       cudaMemcpyDeviceToDevice);

            //DISPATCH CUB
            void* d_temp_storage = NULL;
            size_t temp_storage_bytes = 0;
            cub::DeviceSegmentedSort::SortPairs(d_temp_storage, temp_storage_bytes, sortCopy,
                                                sortCopy, payloadCopy, payloadCopy, totalSegLength,
                                                segmentCount, m_segments, m_segments + 1);
            cudaMalloc(&d_temp_storage, temp_storage_bytes);
            cub::DeviceSegmentedSort::SortPairs(d_temp_storage, temp_storage_bytes, sortCopy,
                                                sortCopy, payloadCopy, payloadCopy, totalSegLength,
                                                segmentCount, m_segments, m_segments + 1);
   ...
   private:
    ...
    bool DispatchInitSegmentsRandomLengthRandomValue(uint32_t* segInitInfo,
        ...
        InitSegLengthsRandom<<<4096, 64>>>(m_segments, m_segInitInfo, seed, maxTotalSegLength,
                                           maxSegLength);
    ...
    bool ValidateBinning(uint32_t totalSegCount, uint32_t totalSegLength, uint32_t totalBinCount,
                         bool verbose) {
        ...
        ValidateBinningRandomSegLengths<<<4096, 64>>>(
            m_segments, SplitSortInternal::GetBinOffsetsPointer(m_tempMem, parts, totalSegCount),
            m_segInfoValidate, SplitSortInternal::GetPackedSegCountsPointer(m_tempMem, parts),
            m_errCount, totalSegCount, totalSegLength,
    ...
    bool ValidateSegSortRandomLength(uint32_t totalSegCount, uint32_t totalSegLength,
                                     bool verbose) {
        uint32_t errCount[1];
        cudaMemset(m_errCount, 0, sizeof(uint32_t));
        ValidateRandomLengthSegments<<<4096, 64>>>(m_sort, m_payloads, m_segments, m_errCount,
                                                   totalSegLength, totalSegCount, verbose);
        cudaMemcpy(&errCount, m_errCount, sizeof(uint32_t), cudaMemcpyDeviceToHost);
        return !errCount[0];
    }

    bool ValidateFull(uint32_t* sortCopy, K* payloadCopy, uint32_t totalSegLength) {
        uint32_t errCount[1];
        cudaMemset(m_errCount, 0, sizeof(uint32_t));
        ValidateSegSortSanity<<<256, 256>>>(m_sort, sortCopy, m_payloads, payloadCopy, m_errCount,
                                            totalSegLength);
        cudaMemcpy(&errCount, m_errCount, sizeof(uint32_t), cudaMemcpyDeviceToHost);
        return !errCount[0];
    }
