...
struct policy_hub_t {
    ...
    struct policy_t : cub::ChainedPolicy<300, policy_t, policy_t> {
        ...
        using OnesweepPolicy = cub::AgentRadixSortOnesweepPolicy<
            512, 15, DominantT, 1, cub::RADIX_RANK_MATCH_EARLY_COUNTS_ANY,
            cub::BLOCK_SCAN_RAKING_MEMOIZE, cub::RADIX_SORT_STORE_DIRECT, ONESWEEP_RADIX_BITS>;
        ...
        using ScanPolicy = cub::AgentScanPolicy<512, 23, OffsetT, cub::BLOCK_LOAD_WARP_TRANSPOSE,
                                                cub::LOAD_DEFAULT, cub::BLOCK_STORE_WARP_TRANSPOSE,
                                                cub::BLOCK_SCAN_RAKING_MEMOIZE>;

        static constexpr int SINGLE_TILE_RADIX_BITS = (sizeof(KeyT) > 1) ? 6 : 5;

        using SingleTilePolicy =
            cub::AgentRadixSortDownsweepPolicy<256, 19, DominantT, cub::BLOCK_LOAD_DIRECT,
                                               cub::LOAD_LDG, cub::RADIX_RANK_MEMOIZE,
                                               cub::BLOCK_SCAN_WARP_SCANS, SINGLE_TILE_RADIX_BITS>;
    };
...
class CubDispatcher {
   ...
   public:
    CubDispatcher(bool keysOnly, uint32_t size) : k_keysOnly(keysOnly), k_maxSize(size) {
    ...
    void BatchTimingCubDeviceRadixSortKeys(uint32_t size, uint32_t batchCount, uint32_t seed,
        ...
        cub::DeviceRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, m_sort, m_sort, size);
        ...
        for (uint32_t i = 0; i <= batchCount; ++i) {
            ...
            cub::DeviceRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, m_sort, m_sort,
                                           size);
    ...
    void BatchTimingCubDeviceRadixSortPairs(uint32_t size, uint32_t batchCount, uint32_t seed,
                                            ENTROPY_PRESET entropyPreset) {
        if (k_keysOnly) {
        ...
        void* d_temp_storage = NULL;
        size_t temp_storage_bytes = 0;
        cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, m_sort, m_sort,
                                        m_payloads, m_payloads, size);
        cudaMalloc(&d_temp_storage, temp_storage_bytes);
    ...
    void BatchTimingCubOneSweepKeys(uint32_t size, uint32_t batchCount, uint32_t seed,
                                    ENTROPY_PRESET entropyPreset) {
        ...
        using dispatch_t =
            cub::DispatchRadixSort<false, uint32_t, cub::NullType, uint32_t, policy_t>;
        ...
        dispatch_t::Dispatch(temp_storage, temp_storage_bytes, d_keys, d_values, size, begin_bit,
                             end_bit, false, 0);
        ...
        for (uint32_t i = 0; i <= batchCount; ++i) {
            ...
            cudaDeviceSynchronize();
            cudaEventRecord(start);
            dispatch_t::Dispatch(temp_storage, temp_storage_bytes, d_keys, d_values, size,
                                 begin_bit, end_bit, false, 0);
            cudaEventRecord(stop);
            cudaEventSynchronize(stop);
    ...
    void BatchTimingCubOneSweepPairs(uint32_t size, uint32_t batchCount, uint32_t seed,
                                     ENTROPY_PRESET entropyPreset) {
        if (k_keysOnly) {
            printf("Error, object was intialized for keys only");
            return;
